...
def build_tree_id3(inputs, split_candidates=None):
    ...
    # 借助偏函数partial来调用partition_entropy_by函数。inputs作为第一个参数，split_candidates中的元素作为第二个参数传入
...
def tree_classify(tree, input):
    ...
    # 如果特征名称在input中不存在(待预测数据可能缺失某些特征)，则get方法返回None(NoneType)；而input[attribute]将会报错
    ...
    # 递归调用，直到找到了最终叶子节点
    return tree_classify(subtree, input)

def forest_classify(trees, input):
    votes = [tree_classify(tree, input) for tree in trees]
    vote_counts = col.Counter(votes)
