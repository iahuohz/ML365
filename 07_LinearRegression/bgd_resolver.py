...
def batch_gradient_descent(target_fn, gradient_fn, init_W, X, Y, learning_rate=0.001, tolerance=1e-9):
    """支持多变量的批量梯度下降法"""
...
def batch_gradient_descent_rsquare(theta, X, y):
    ssRes = np.sum((X.dot(theta) - y) ** 2)
